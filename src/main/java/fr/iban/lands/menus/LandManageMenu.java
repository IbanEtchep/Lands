...
public class LandManageMenu extends Menu {
    ...
    public LandManageMenu(Player player, LandsPlugin plugin, Land land) {
        super(player);
        this.land = land;
        this.plugin = plugin;
    }

    public LandManageMenu(Player player, LandsPlugin plugin, Land land, Menu previousMenu) {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (displayNameEquals(current, "§2Permissions")) {
            new TrustsManageMenu(player, plugin, land, this).open();
        } else if (displayNameEquals(current, "§2Tronçons protégés")) {
            new ClaimsManageMenu(player, plugin, land, this).open();
        } else if (displayNameEquals(current, "§2Paramètres")) {
            new LandSettingsMenu(player, land, plugin, this).open();
        } else if (displayNameEquals(current, "§2Bannissements")) {
            new BansManageMenu(player, plugin, land, this).open();
        } else if (displayNameEquals(current, "§2Sous-territoires")) {
            new SubLandMainMenu(player, plugin, land, previousMenu).open();
            ...
            if (land instanceof SubLand subLand) {
                ...
                CuboidSelector selector = new CuboidSelector(player, plugin, subLand, () -> {
    ...
    public void setMenuItems() {
        if (land.getType() != LandType.SUBLAND) {
            inventory.setItem(10, new ItemBuilder(Head.CHEST_DIRT.get())
                    .setDisplayName("§2Tronçons protégés")
