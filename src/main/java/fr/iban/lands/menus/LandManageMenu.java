...
public class LandManageMenu extends Menu {
    ...
    public LandManageMenu(
            Player player, LandsPlugin plugin, LandManager manager, Land land, Menu previousMenu) {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (displayNameEquals(current, "§2Permissions")) {
            ...
            new SubLandMainMenu(
                    player, plugin, new ArrayList<>(land.getSubLands().values()), land, previousMenu)
                    .open();
            ;
    ...
    public void setMenuItems() {
        if (land.getType() != LandType.SUBLAND) {
            inventory.setItem(
                    10,
                    new ItemBuilder(Head.CHEST_DIRT.get())
        ...
        inventory.setItem(
                14,
                new ItemBuilder(Head.FIREBALL.get())
        ...
        if (land.getType() != LandType.SUBLAND && player.hasPermission("lands.sublands")) {
            inventory.setItem(
                    27,
                    new ItemBuilder(Head.DIRT_S.get())
